...
export const api = {
  ...
  loyalty: {
    getTasks: () => fetchClient<{ tasks: any[] }>('/tasks'),
    getVouchers: () => fetchClient<{ vouchers: any[] }>('/vouchers'),
    completeTask: (taskId: string) =>
      fetchClient<{ user: any; tasks: any[] }>('/tasks/complete', { method: 'POST', body: JSON.stringify({ taskId }) }),
    redeemVoucher: (voucherId: string) =>
      fetchClient<{ user: any; vouchers: any[] }>('/vouchers/redeem', { method: 'POST', body: JSON.stringify({ voucherId }) }),
  },

  subscribeAIPro: () =>
    fetchClient<{ user: any; message: string }>('/ai/subscribe', { method: 'POST', body: JSON.stringify({ tier: 'PRO' }) }),
};
