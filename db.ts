...
import { MOCK_PRODUCTS, MOCK_STREAMS, MOCK_ALL_USERS, MOCK_FEED_POSTS, MOCK_REVIEWS, MOCK_DISCOUNT_CODES, MOCK_SHIPPING_OPTIONS, DEFAULT_LOYALTY_TASKS, DEFAULT_LOYALTY_VOUCHERS } from './data';
  ...
  private load(): Schema {
    if (fs.existsSync(DB_FILE)) {
      try {
        ...
        // Migration: Ensure tasks and vouchers exist
        ...
        data.users = data.users.map((u: any) => ({
          ...
          twoFactorSecret: '',
          completedLoyaltyTaskIds: u.completedLoyaltyTaskIds || [],
          redeemedVoucherIds: u.redeemedVoucherIds || [],
          ...u,
          wallet: {
            balance: u.balance || 0,
            pendingBalance: 0,
    ...
    const defaultData: Schema = {
      ...
      departments: [],
      branches: [],
      tasks: DEFAULT_LOYALTY_TASKS.map((t) => ({ ...t })),
      vouchers: DEFAULT_LOYALTY_VOUCHERS.map((v) => ({ ...v })),
      globalConfig: {
        platformFeeRate: 0.05,
        defaultVatRate: 0.08,
        personalIncomeTaxRate: 0.015,
