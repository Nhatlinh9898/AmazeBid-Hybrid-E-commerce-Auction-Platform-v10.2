...
import { db } from '../db';
import { DEFAULT_LOYALTY_TASKS, DEFAULT_LOYALTY_VOUCHERS } from '../data';
import { PricingService } from '../services/pricingService';
import PaymentService from '../services/PaymentService';
import { OrderStatus, ItemType, WalletTransaction, EscrowItem, User, UserTier, AISubscriptionTier, Product, Voucher } from '../types';
...
const paymentRateLimiter = rateLimit({
  ...
  max: 5, // Limit each IP to 5 payment intents per 15 minutes
  message: {
    status: 'error',
...
// Rate limiting for webhook endpoints (more lenient for legitimate webhooks)
const webhookRateLimiter = rateLimit({
  windowMs: 1 * 60 * 1000, // 1 minute
  max: 50, // Allow 50 webhook events per minute
  message: {
    status: 'error',
...
function getAuthPayload(req: Request): AuthPayload | null {
  ...
  if (!decoded || (decoded as { error?: string }).error || !(decoded as { id?: string }).id) return null;
  return decoded as AuthPayload;
}

function isAdminPayload(payload: AuthPayload): boolean {
  if (payload.id === 'admin_root') return true;
  const adminEmail = SecurityService.getAdminEmail().toLowerCase().trim();
  if (payload.email && payload.email.toLowerCase().trim() === adminEmail) return true;
  const users = db.get('users') || [];
  const u = users.find((x: User) => x.id === payload.id);
  return u?.role === 'ADMIN';
}
...
function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!p) {
    return res.status(401).json({
      status: 'error',
      action: 'auth',
      data: { message: 'Yêu cầu đăng nhập. Gửi header Authorization: Bearer <token>.' },
      ui: { update: false, elements: null }
    });
  }
  (req as Request & { auth: AuthPayload }).auth = p;
  next();
}

function emitBidUpdated(req: Request, productId: string, product: unknown) {
  try {
    const io = req.app.get('io') as IOServer | undefined;
    if (io) io.to(productId).emit('bid:updated', { productId, product });
  } catch (e) {
    console.warn('[v3] bid socket emit skipped:', e);
  }
}
...
router.put('/products/:id', requireAuth, async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const products = db.get('products') || [];
    const existing = products.find((p: Product) => p.id === id);
    if (!existing) return sendError(res, 'Không tìm thấy sản phẩm', 'update_product');

    if (!isAdminPayload(auth) && existing.sellerId !== auth.id) {
      return res.status(403).json({
        status: 'error',
        action: 'update_product',
        data: { message: 'Bạn không có quyền chỉnh sửa sản phẩm này.' },
        ui: { update: false, elements: null }
      });
    }
...
router.delete('/products/:id', requireAuth, async (req, res) => {
  try {
    ...
    const products = db.get('products') || [];
    const existing = products.find((p: Product) => p.id === id);
    if (!existing) return sendError(res, 'Không tìm thấy sản phẩm', 'delete_product');

    if (!isAdminPayload(auth) && existing.sellerId !== auth.id) {
      return res.status(403).json({
        status: 'error',
        action: 'delete_product',
        data: { message: 'Bạn không có quyền xóa sản phẩm này.' },
        ui: { update: false, elements: null }
      });
    }
  ...
  try {
    ...
    if (userId !== auth.id) {
      return res.status(403).json({
        status: 'error',
        action: 'place_bid',
        data: { message: 'userId không khớp phiên đăng nhập.' },
        ui: { update: false, elements: null }
      });
    }

    const users = db.get('users') || [];
    const bidder = users.find((u: User) => u.id === auth.id);
    const displayName = bidder?.fullName || auth.email || 'Bidder';
    ...
    const updatedProduct = {
      ...
      bidHistory: [
        { id: `bid_${Date.now()}`, userId: auth.id, userName: displayName, amount, timestamp: new Date().toISOString() },
        ...(product.bidHistory || [])
      ]
    };
    ...
    emitBidUpdated(req, productId, updatedProduct);
    ...
    if (!user) {
      ...
      if (user.role !== 'ADMIN') {
        ...
        await db.update('users', (prev) =>
          (prev || []).map((u: User): User => (u.id === user!.id ? { ...u, role: 'ADMIN' as const } : u))
        );
      }
    }
  } else if (user) {
...
router.post('/auth/reset-token', requireAuth, async (req, res) => {
  try {
    const auth = (req as Request & { auth: AuthPayload }).auth;
    ...
    if (userId !== auth.id) {
      return res.status(403).json({
        status: 'error',
        action: 'reset_token',
        data: { message: 'Chỉ có thể vô hiệu hóa phiên đăng nhập của chính bạn.' },
        ui: { update: false, elements: null }
      });
    }
...
router.post('/ai/charge', async (req, res) => {
    try {
      ...
      if (user.role === 'ADMIN' || isPro) {
          ...
          if (!user.aiUsage) user.aiUsage = { totalTokens: 0, totalRequests: 0, lastRequestDate: new Date().toISOString(), usageHistory: [] };
      ...
      if (!user.aiUsage) user.aiUsage = { totalTokens: 0, totalRequests: 0, lastRequestDate: new Date().toISOString(), usageHistory: [] };
      user.aiUsage.totalRequests++;
      // Note: estimatedCost is not part of AIUsageStats interface
...
router.post('/admin/withdraw', async (req, res) => {
  try {
    ...
    if (!wallet || wallet.balance < amount) {
      return sendError(res, 'Insufficient funds', 'withdraw_insufficient');
    }
...
router.get('/orders/seller/:sellerId', (req, res) => {
  try {
    ...
    sendError(res, 'Failed to fetch seller orders', 'get_seller_orders');
  }
});
...
router.post('/posts', requireAuth, async (req, res) => {
  try {
    const auth = (req as Request & { auth: AuthPayload }).auth;
    const users = db.get('users') || [];
    const u = users.find((x: User) => x.id === auth.id);
    const post = req.body;
    ...
    post.userId = auth.id;
    post.userName = u?.fullName || auth.email || 'User';
    post.userAvatar = u?.avatar || '';
...
router.put('/products/:id/status', requireAuth, async (req, res) => {
  try {
    const auth = (req as Request & { auth: AuthPayload }).auth;
    const { id } = req.params;
    const products = db.get('products') || [];
    const existing = products.find((p: Product) => p.id === id);
    if (!existing) return sendError(res, 'Không tìm thấy sản phẩm', 'update_product_status');

    if (!isAdminPayload(auth) && existing.sellerId !== auth.id) {
      return res.status(403).json({
        status: 'error',
        action: 'update_product_status',
        data: { message: 'Bạn không có quyền cập nhật trạng thái sản phẩm này.' },
        ui: { update: false, elements: null }
      });
    }
...
router.post('/create-payment-intent', paymentRateLimiter, async (req, res) => {
  try {
...
router.post('/webhook/stripe', webhookRateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!event) {
      console.warn('[v3] Webhook signature verification failed or skipped');
      // In development, you might want to process anyway
      // For production, return 400 to reject unverified webhooks
    }

    // Process the webhook event
    const result = await PaymentService.processWebhook(event || req.body);
    
    res.json({ received: result.received, processed: result.processed });
  } catch (error: any) {
...
router.get('/users/:id/wallet', (req, res) => {
  try {
    ...
    if (!user && id === 'admin_root') {
      ...
      // Note: SystemWallet doesn't have escrowItems property in the interface
...
router.get('/tasks', async (req, res) => {
  try {
    ...
    if (!tasks.length) {
      await db.update('tasks', () => DEFAULT_LOYALTY_TASKS.map((t) => ({ ...t })));
      tasks = db.get('tasks') || [];
    }
    const p = getAuthPayload(req);
    ...
    if (p) {
      const users = db.get('users') || [];
      const u = users.find((x: User) => x.id === p.id);
      (u?.completedLoyaltyTaskIds || []).forEach((id: string) => completedIds.add(id));
    }
    const out = tasks.map((t: { id: string; title?: string; points?: number; type?: string; isCompleted?: boolean }) => ({
      ...t,
      isCompleted: completedIds.has(t.id)
    }));
    sendSuccess(res, { tasks: out }, 'get_tasks');
  } catch (error: any) {
    sendError(res, error.message, 'get_tasks');
  }
});

router.post('/tasks/complete', requireAuth, async (req, res) => {
  try {
    ...
    if (!taskId) {
      return res.status(400).json({
        ...
        data: { message: 'Thiếu taskId' },
        ui: { update: false, elements: null }
      });
    }
    const catalog = db.get('tasks') || [];
    const task = catalog.find((t: { id: string }) => t.id === taskId);
    if (!task) {
      return res.status(404).json({
        status: 'error',
        action: 'complete_task',
        data: { message: 'Nhiệm vụ không tồn tại' },
        ui: { update: false, elements: null }
      });
    }
    const auth = (req as Request & { auth: { id: string } }).auth;
    await db.update('users', (prev) => {
      const users = [...(prev || [])];
      const idx = users.findIndex((u: User) => u.id === auth.id);
      if (idx === -1) throw new Error('Người dùng không tồn tại');
      const u: User = { ...users[idx] };
      const completed = new Set(u.completedLoyaltyTaskIds || []);
      if (!completed.has(taskId)) {
        completed.add(taskId);
        u.completedLoyaltyTaskIds = Array.from(completed);
        u.points = (u.points || 0) + (Number((task as { points?: number }).points) || 0);
      }
      users[idx] = u;
      return users;
    });
    const users = db.get('users') || [];
    const user = users.find((x: User) => x.id === auth.id);
    const completedSet = new Set(user?.completedLoyaltyTaskIds || []);
    const tasksOut = catalog.map((t: { id: string }) => ({
      ...t,
      isCompleted: completedSet.has(t.id)
    }));
    sendSuccess(res, { user, tasks: tasksOut }, 'complete_task');
  } catch (error: any) {
    sendError(res, error.message, 'complete_task');
  }
});
...
router.get('/vouchers', async (req, res) => {
  try {
    ...
    if (!vouchers.length) {
      await db.update('vouchers', () => DEFAULT_LOYALTY_VOUCHERS.map((v) => ({ ...v })));
    ...
    if (p) {
      const users = db.get('users') || [];
      const u = users.find((x: User) => x.id === p.id);
      (u?.redeemedVoucherIds || []).forEach((id: string) => redeemedIds.add(id));
    }
    const out = vouchers.map((v: { id: string }) => ({
      ...v,
      isRedeemed: redeemedIds.has(v.id)
    }));
    sendSuccess(res, { vouchers: out }, 'get_vouchers');
  } catch (error: any) {
    sendError(res, error.message, 'get_vouchers');
  }
});

router.post('/vouchers/redeem', requireAuth, async (req, res) => {
  try {
    const { voucherId } = (req.body || {}) as { voucherId?: string };
    if (!voucherId) {
      return res.status(400).json({
        status: 'error',
        action: 'redeem_voucher',
        data: { message: 'Thiếu voucherId' },
        ui: { update: false, elements: null }
      });
    }
    const catalog = db.get('vouchers') || [];
    const voucher = catalog.find((v: { id: string }) => v.id === voucherId);
    if (!voucher) {
      return res.status(404).json({
        status: 'error',
        action: 'redeem_voucher',
    ...
    const auth = (req as Request & { auth: { id: string } }).auth;
    const required = Number((voucher as { pointsRequired?: number }).pointsRequired) || 0;
    ...
    await db.update('users', (prev) => {
      ...
      if (idx === -1) throw new Error('Người dùng không tồn tại');
      const u: User = { ...users[idx] };
      const redeemed = new Set(u.redeemedVoucherIds || []);
      if (redeemed.has(voucherId)) {
        users[idx] = u;
        return users;
      }
      ...
      redeemed.add(voucherId);
      u.redeemedVoucherIds = Array.from(redeemed);
      u.points = points - required;
      const owned = [...(u.vouchers || [])];
      owned.push({ ...(voucher as Voucher) });
      u.vouchers = owned;
      users[idx] = u;
      return users;
    });
    ...
    const users = db.get('users') || [];
    const user = users.find((x: User) => x.id === auth.id);
    const redeemedSet = new Set(user?.redeemedVoucherIds || []);
    const vouchersOut = catalog.map((v: { id: string }) => ({
      ...v,
      isRedeemed: redeemedSet.has(v.id)
    }));
    sendSuccess(res, { user, vouchers: vouchersOut }, 'redeem_voucher');
  } catch (error: any) {
    if (error.message === 'INSUFFICIENT_POINTS') {
      return res.status(400).json({
        status: 'error',
        action: 'redeem_voucher',
        data: { message: 'Không đủ điểm!' },
        ui: { update: false, elements: null }
      });
    }
    sendError(res, error.message, 'redeem_voucher');
  }
});
