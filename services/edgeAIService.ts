...
class EdgeAIService {
  ...
  constructor() {
    ...
    // Silence ONNX Runtime and Transformers.js logs
    (env as any).logLevel = 'error';
    if (env.backends && env.backends.onnx) {
      env.backends.onnx.logLevel = 'error';
      // Disable noisy logs from ONNX Runtime C++ core if possible
      (env.backends.onnx as any).debug = false;
      if ((env.backends.onnx as any).wasm) {
        (env.backends.onnx as any).wasm.logLevel = 'error';
      }
    }
    
    // Explicitly set ONNX log levels if they exist in this version's schema
    if ((env as any).onnx) {
      (env as any).onnx.logLevel = 'error';
      (env as any).onnx.debug = false;
    }

    // Prevents warnings from showing up when imported in Node.js
  ...
  async getCapabilities() {
    ...
    return {
      ...
      memory,
      cores,
      tier: hasWebGPU ? 'HIGH' : ((typeof cores === 'number' && cores > 4) ? 'MEDIUM' : 'LOW')
    };
  }
