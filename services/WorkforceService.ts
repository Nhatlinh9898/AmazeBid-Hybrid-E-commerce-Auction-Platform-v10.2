...
class WorkforceService {
  ...
  // Ghi nhận chấm công
  async recordTime(record: Omit<TimeRecord, 'id' | 'timestamp'>) {
    ...
    const staffMember = this.staff.find(s => s.id === record.staffId);
    if (staffMember) {
      const timeRecords = staffMember.timeRecords || [];
      timeRecords.push(newRecord);
      await this.updateStaff(record.staffId, { timeRecords });
    }
    return newRecord;
  }

  // Tính toán lương thưởng dựa trên đánh giá khách hàng (KPI)
  async calculateDynamicSalary(staffId: string, storeReviews: Review[]) {
    const staffMember = this.staff.find(s => s.id === staffId);
    if (!staffMember) return 0;

    // Lọc các đánh giá liên quan đến nhân viên này (nếu có mention) hoặc dùng trung bình của store cho toàn bộ team
    // Ở đây ta dùng trung bình của store như một chỉ số KPI tập thể nếu không có đánh giá riêng
    ...
    // Thưởng: Mỗi sao trên 3.0 sẽ được thưởng 5% lương cơ bản
    const bonusMultiplier = Math.max(0, (avgRating - 3) * 0.05);
    const bonus = baseSalary * bonusMultiplier;
    
    // Cập nhật kpiScore cục bộ
    const kpiScore = avgRating * 20; // Chuyển thang 5 sang 100
    await this.updateStaff(staffId, { kpiScore });
