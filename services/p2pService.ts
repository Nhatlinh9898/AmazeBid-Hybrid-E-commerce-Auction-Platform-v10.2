...
class P2PService {
  ...
  private initGun(server?: any) {
    ...
    console.error = (...args: any[]) => {
      ...
      if (typeof message === 'string' && 
          (message.includes('WebSocket connection to') || 
           message.includes('gun-manhattan.herokuapp.com') ||
           message.includes('localhost:3000/gun'))) {
        // Suppress GunDB connection errors - they're expected in development
        return;
      }
    ...
    if (typeof window === 'undefined') {
      ...
      this.gun.on('out', function(this: any, msg: any) {
        ...
        if (msg.peer && sentinel.getBlacklist().includes(msg.peer.id)) {
  ...
  /**
   * Get the Gun instance for external access
   */
  getGunInstance() {
    this.initGun();
    return this.gun;
   ...
   * Phát tán sự kiện blockchain qua mạng P2P để đồng bộ trạng thái
   */
  publishBlockchainEvent(eventType: string, data: any) {
    this.initGun();
    const blockchainNode = this.gun.get('blockchain-events');
    
    const eventData = {
      type: eventType,
      data: data,
      timestamp: Date.now(),
      publisher: this.user?.is?.pub || 'server'
    };
    
    blockchainNode.get(eventType).put(eventData);
    console.log(`[P2P] Blockchain event published: ${eventType}`);
  }

  /**
   * Theo dõi sự kiện blockchain từ mạng P2P
   */
  subscribeToBlockchainEvents(eventType: string, callback: (data: any) => void) {
    this.initGun();
    this.gun.get('blockchain-events').get(eventType).on((data: any) => {
      if (data && callback) {
        callback(data);
      }
    });
  ...
  /**
   * Phát tán trạng thái giao dịch escrow qua P2P
   */
  publishEscrowUpdate(orderId: string, status: string, amount: number) {
    this.publishBlockchainEvent('escrow-update', {
      orderId,
      status,
      amount,
      timestamp: Date.now()
  ...
  /**
   * Phát tán trạng thái verification qua P2P
   */
  publishVerificationUpdate(userId: string, status: string) {
    this.publishBlockchainEvent('verification-update', {
      userId,
      status,
      timestamp: Date.now()
    });
