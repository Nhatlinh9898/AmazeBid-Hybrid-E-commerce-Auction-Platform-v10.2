...
const UserProfile: React.FC<UserProfileProps> = ({ isOpen, onClose, myProducts = [], myPosts = [], onOpenWorkLogin }) => {
  const { user, logout, updateProfile, resetToken, setup2FA, confirm2FA, toggle2FA, linkSocialAccount, refreshUser } = useAuth();
  ...
  const handleSubscribeAIPro = async () => {
    ...
    try {
      const data = await api.subscribeAIPro();
      alert(data.message || 'Đăng ký gói AI Pro thành công.');
      if (data.user) {
        await refreshUser();
      } else {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [t, v] = await Promise.all([api.loyalty.getTasks(), api.loyalty.getVouchers()]);
        if (!cancelled) {
          setTasks(t.tasks || []);
          setVouchers(v.vouchers || []);
        }
      } catch (e) {
        console.warn('Loyalty fetch failed:', e);
        if (!cancelled) {
          setTasks([]);
          setVouchers([]);
        }
  ...
  const completeTask = async (taskId: string) => {
    try {
      const data = await api.loyalty.completeTask(taskId);
      setTasks(data.tasks || []);
      await refreshUser();
    } catch (e: any) {
      alert(e.message || 'Không thể hoàn thành nhiệm vụ.');
    }
  };

  const redeemVoucher = async (voucherId: string) => {
    try {
      const data = await api.loyalty.redeemVoucher(voucherId);
      setVouchers(data.vouchers || []);
      await refreshUser();
      alert('Đổi voucher thành công!');
    } catch (e: any) {
      alert(e.message || 'Không đủ điểm!');
    }
  };
        ...
        <div className="w-full md:w-64 bg-gray-50 border-r border-gray-200 p-6 flex flex-col">
            <div className="text-center mb-6">
                ...
                <div className="mt-2 flex flex-col items-center gap-1.5">
                    <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-bold">
                        <ShieldCheck size={10} /> Verified
                    </div>
                    {(() => {
                      const reputation = TrustScore.getReputationLevel(user.reputation || 0, (myProducts?.length || 0) + (myPosts?.length || 0) + 10); // Mocking transaction count
                      return (
                        <div className={`inline-flex items-center gap-1 px-2 py-0.5 bg-white border border-gray-100 rounded-full text-[10px] font-bold shadow-sm ${reputation.color}`}>
                           {reputation.label}
                        </div>
                      );
                    })()}
                            ...
                            {(vouchers || []).map(voucher => (
                                <div key={voucher.id} className="p-4 border rounded-xl space-y-2">
                                    ...
                                    <button 
                                        ...
                                        className={`w-full py-2 rounded-lg text-sm font-bold ${voucher.isRedeemed ? 'bg-gray-200 text-gray-500' : 'bg-green-600 text-white'}`}
                                    >
                                        {voucher.isRedeemed ? 'Đã đổi' : `Đổi với ${voucher.pointsRequired} điểm`}
                                    </button>
                                </div>
                            ))}
                        </div>
