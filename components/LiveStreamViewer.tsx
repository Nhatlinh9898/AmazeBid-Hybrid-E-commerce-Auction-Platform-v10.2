...
const LiveStreamViewer: React.FC<LiveStreamViewerProps> = ({ stream, products, onClose, onPlaceBid, onAddToCart, isHost = false }) => {
  ...
  const [lastBidder, setLastBidder] = React.useState<{ name: string, amount: number } | null>(null);
  const [showBidAnimation, setShowBidAnimation] = React.useState(false);
  ...
  React.useEffect(() => {
    if (featuredProduct) {
        ...
        if (updated && updated !== featuredProduct) {
            // Check if new bid
            if (updated.type === ItemType.AUCTION && (updated.currentBid || 0) > (featuredProduct.currentBid || 0)) {
                const latestBid = updated.bidHistory?.[updated.bidHistory.length - 1];
                if (latestBid) {
                    setLastBidder({ name: latestBid.userName, amount: latestBid.amount });
                    setShowBidAnimation(true);
                    setTimeout(() => setShowBidAnimation(false), 3000);
                }
            }
            const timer = setTimeout(() => {
      ...
      <div className="relative flex-1 bg-gray-900 flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-4 left-4 flex items-center gap-3 z-20">
            ...
            <div className="bg-black/40 backdrop-blur px-3 py-1.5 rounded-full flex items-center gap-2">
                ...
                <span className="text-xs border-l border-gray-100 pl-2 ml-1">
                    {isHost ? 'Đang phát' : `${stream?.viewerCount.toLocaleString()} xem`}
                </span>
            </div>
        </div>

        {/* Real-time Bid Animation Overlay */}
        <AnimatePresence>
            {showBidAnimation && lastBidder && (
                <motion.div 
                ...
                >
                    <div className="bg-gradient-to-r from-orange-600 to-yellow-500 p-6 rounded-3xl shadow-2xl border-4 border-white flex flex-col items-center gap-2 animate-bounce">
                        <Trophy size={48} className="text-white drop-shadow-lg" />
                        <div className="text-center">
                            <p className="text-xs font-black text-white uppercase tracking-widest opacity-80">🔥 Leading Bid!</p>
                            <h4 className="text-2xl font-black text-white uppercase tracking-tighter leading-none">{lastBidder.name}</h4>
                            <p className="text-4xl font-black text-white mt-1">${lastBidder.amount.toLocaleString()}</p>
                        </div>
                    </div>
                </motion.div>
            )}
        ...
        {/* Quick Bid Floating Action (Directly on Video) */}
        {featuredProduct?.type === ItemType.AUCTION && !isHost && (
            <div className="absolute bottom-6 left-6 z-30">
                <motion.button
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => handleQuickBid(featuredProduct.stepPrice || 10)}
                    className="bg-red-600 text-white px-6 py-3 rounded-2xl font-black shadow-xl shadow-red-900/40 flex items-center gap-3 border-2 border-white/20"
                >
                    <Gavel size={20} />
                    <span>Đấu giá ngay +${featuredProduct.stepPrice || 10}</span>
                </motion.button>
            </div>
        )}
