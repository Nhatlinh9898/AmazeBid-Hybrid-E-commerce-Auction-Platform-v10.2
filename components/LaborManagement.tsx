...
export const LaborManagement: React.FC<LaborManagementProps> = ({ ownerId, onTabChange }) => {
  ...
  const [activeTab, setActiveTab] = React.useState<'STAFF' | 'TIMEKEEPING'>('STAFF');
  const [isRecordingTime, setIsRecordingTime] = React.useState(false);
  const [timekeepingMethod, setTimekeepingMethod] = React.useState<'QR' | 'GEO' | null>(null);
  const [dynamicSalaries, setDynamicSalaries] = React.useState<Record<string, number>>({});
  ...
  React.useEffect(() => {
    ...
    const unsubWorkforce = workforceService.subscribe((allStaff) => {
      if (selectedStoreId) {
        const storeStaff = allStaff.filter(s => s.storeId === selectedStoreId);
        setStaff(storeStaff);
        
        // Calculate dynamic salaries
        const fetchSalaries = async () => {
          const store = await storeService.getStoreById(selectedStoreId);
          if (store) {
            const salaries: Record<string, number> = {};
            for (const member of storeStaff) {
              const pay = await workforceService.calculateDynamicSalary(member.id, store.reviews || []);
              salaries[member.id] = pay;
            }
            setDynamicSalaries(salaries);
          }
        };
        fetchSalaries();
      }
  ...
  const startEditStaff = (member: StoreStaff) => {
    ...
    setStaffForm({
      ...
      kpiScore: member.kpiScore || 0,
      overtimeHours: member.overtimeHours || 0
    });
    setIsAddingStaff(true);
  };
  ...
  const handleRecordTime = async (staffId: string, method: 'QR' | 'GEOLOCATION') => {
    setIsRecordingTime(true);
    try {
      // Mock location for Geolocation
      let location = undefined;
      if (method === 'GEOLOCATION') {
        location = { lat: 10.762622, lng: 106.660172 }; // Mock HCM City center
      }

      const lastRecord = staff.find(s => s.id === staffId)?.timeRecords?.slice(-1)[0];
      const type = lastRecord?.type === 'CHECK_IN' ? 'CHECK_OUT' : 'CHECK_IN';

      await workforceService.recordTime({
        staffId,
      ...
      alert(`${type === 'CHECK_IN' ? 'Vào ca' : 'Tan ca'} thành công!`);
    } catch (error) {
      console.error('Timekeeping error:', error);
    } finally {
      setIsRecordingTime(false);
      setTimekeepingMethod(null);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
        ...
        <div className="flex items-center gap-4 min-w-max">
          ...
          <div className="flex gap-2">
            {stores.map(store => (
              ...
              >
                {store.type === 'BRANCH' ? `Chi nhánh: ${store.name}` : `Cửa hàng: ${store.name}`}
              </button>
            ))}
            {stores.length === 0 && (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-2 border-b border-gray-100 bg-gray-50 flex">
          <button 
            onClick={() => setActiveTab('STAFF')}
            className={`flex-1 py-3 text-xs font-black uppercase tracking-widest transition-all ${
              activeTab === 'STAFF' ? 'text-blue-600 bg-white border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            Quản lý Nhân sự
          </button>
          <button 
            ...
            className={`flex-1 py-3 text-xs font-black uppercase tracking-widest transition-all ${
              activeTab === 'TIMEKEEPING' ? 'text-blue-600 bg-white border-b-2 border-blue-600' : 'text-gray-400 hover:text-gray-600'
            }`}
          >
            Chấm công (QR/GEO)
          </button>
        ...
        {activeTab === 'STAFF' ? (
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="text-gray-400 uppercase text-[10px] tracking-wider border-b border-gray-100">
                    ...
                    <th className="p-4">Lương (Review Bonus)</th>
                        ...
                        {member.baseSalary ? (
                          <div className="flex flex-col">
                            <div className="flex items-center gap-1.5 text-xs font-black text-gray-900">
                              ...
                              {(dynamicSalaries[member.id] || member.baseSalary).toLocaleString()} đ
                            </div>
                            <div className="flex items-center gap-1 text-[8px] font-black text-blue-500 uppercase">
                              KPI: {member.kpiScore || 0}/100 (Dựa trên Reviews)
                            </div>
                          </div>
                        ) : (
                          <span className="text-[10px] text-gray-400 font-bold">Chưa cấu hình</span>
                        )}
                      </td>
                      <td className="p-4">
          ...
          <div className="p-8 space-y-8 animate-in fade-in slide-in-from-right-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                <h3 className="text-2xl font-black text-gray-900">Chấm công Nhân viên</h3>
                <p className="text-sm text-gray-500">Mã QR và Vị trí địa lý cho chi nhánh {selectedStore?.name}</p>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1 px-3 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase">
                  <CheckCircle2 size={12}/> AI Verification Active
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {/* QR Timekeeping */}
              <div className="bg-gray-50 rounded-[32px] p-8 border border-gray-100 flex flex-col items-center text-center space-y-6">
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-blue-600 shadow-sm">
                  <QrCode size={32} />
                </div>
                <div>
                  <h4 className="font-black text-gray-900">QR Code Chi nhánh</h4>
                  <p className="text-xs text-gray-500 mt-1">Dùng app AmazeBid quét mã này để Check-in/Out</p>
                </div>
                <div className="bg-white p-6 rounded-[32px] shadow-inner border border-gray-100">
                  <img 
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=TIMEKEEPING:${selectedStoreId}`} 
                    alt="Store QR"
                    className="w-40 h-40"
                  />
                </div>
                <button 
                  onClick={() => setTimekeepingMethod('QR')}
                  className="w-full py-4 bg-white border-2 border-blue-600 text-blue-600 rounded-2xl font-black hover:bg-blue-50 transition-all text-sm"
                >
                  Giả lập quét mã QR
                </button>
              </div>
              ...
              <div className="bg-indigo-50 rounded-[32px] p-8 border border-indigo-100 flex flex-col items-center text-center space-y-6">
                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm">
                  <MapPin size={32} />
                </div>
                <div>
                  <h4 className="font-black text-gray-900">Check-in Vị trí (GPS)</h4>
                  <p className="text-xs text-gray-500 mt-1">Hệ thống sẽ xác thực bạn đang ở trong bán kính 100m</p>
                </div>
                <div className="w-full aspect-square max-w-[200px] bg-white rounded-[32px] border border-indigo-100 relative overflow-hidden flex items-center justify-center">
                  <div className="absolute inset-0 bg-[radial-gradient(circle,rgba(79,70,229,0.1)_1px,transparent_1px)] bg-[size:20px_20px]" />
                  <div className="relative">
                    <div className="w-4 h-4 bg-indigo-600 rounded-full animate-ping absolute -inset-0" />
                    <MapPin size={40} className="text-indigo-600 relative z-10" />
                  </div>
                </div>
                <button 
                  onClick={() => setTimekeepingMethod('GEO')}
                  className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all text-sm"
                >
            ...
            {/* Simulating Staff Timekeeping Modal */}
            {timekeepingMethod && (
              <div className="fixed inset-0 z-[500] flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setTimekeepingMethod(null)} />
                <div className="relative bg-white w-full max-w-md rounded-3xl p-6 animate-in zoom-in-95">
                  <h3 className="text-xl font-black mb-4">Cổng Chấm công Nhân viên</h3>
                  <p className="text-sm text-gray-500 mb-6">Chọn nhân viên để thực hiện {timekeepingMethod === 'QR' ? 'Quét mã QR' : 'Xác thực vị trí'}</p>
                  
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2">
                    {staff.map(member => {
                      const lastRecord = member.timeRecords?.slice(-1)[0];
                      const isCheckedIn = lastRecord?.type === 'CHECK_IN';
                      
                      return (
                        <button
                          key={member.id}
                          onClick={() => handleRecordTime(member.id, timekeepingMethod === 'QR' ? 'QR' : 'GEOLOCATION')}
                          disabled={isRecordingTime}
                          className="w-full p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between hover:bg-blue-50 hover:border-blue-200 transition-all text-left"
                        >
                          <div>
                            <p className="font-bold text-gray-900">{member.name}</p>
                            <p className="text-[10px] text-gray-400 font-bold uppercase">{member.role}</p>
                          </div>
                          <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${isCheckedIn ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                            {isCheckedIn ? 'Đang làm' : 'Vắng mặt'}
                          </div>
                        </button>
                      );
                    })}
                  ...
                  <button 
                    onClick={() => setTimekeepingMethod(null)}
                    className="w-full mt-6 py-4 bg-gray-100 text-gray-600 rounded-2xl font-bold hover:bg-gray-200"
                  >
                    Đóng
                  </button>
                </div>
          ...
          <div className="relative bg-white w-full max-w-xl rounded-2xl shadow-2xl p-6 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-xs text-gray-500">Chi nhánh trực thuộc: {formSelectedStore?.type === 'BRANCH' ? formSelectedStore?.name : (formSelectedStore?.name || 'Chưa chọn')}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
                  ...
                  >
                    ...
                    {stores.map(s => (
                      <option key={s.id} value={s.id}>
                        {s.type === 'BRANCH' ? `Chi nhánh: ${s.name}` : `Cửa hàng: ${s.name}`}
                      </option>
                    ))}
                  </select>
                  {formSelectedStore && (
              ...
              <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100/50 space-y-4">
                <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest flex items-center gap-2">
                   <CircleDollarSign size={14}/> Cấu hình Lương & KPI (Thuật toán HR)
                </h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase">Lương cơ bản</label>
                    <input 
                      type="number" 
                      placeholder="0" 
                      className="w-full p-2.5 bg-white rounded-xl outline-none text-sm font-bold border border-blue-100" 
                      value={staffForm.baseSalary || ''}
                      onChange={e => setStaffForm({...staffForm, baseSalary: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase">Điểm KPI (0-100)</label>
                    <input 
                      type="number" 
                      placeholder="0" 
                      className="w-full p-2.5 bg-white rounded-xl outline-none text-sm font-bold border border-blue-100" 
                      value={staffForm.kpiScore || ''}
                      onChange={e => setStaffForm({...staffForm, kpiScore: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-500 uppercase">Giờ làm thêm</label>
                    <input 
                      type="number" 
                      placeholder="0" 
                      className="w-full p-2.5 bg-white rounded-xl outline-none text-sm font-bold border border-blue-100" 
                      value={staffForm.overtimeHours || ''}
                      onChange={e => setStaffForm({...staffForm, overtimeHours: parseFloat(e.target.value) || 0})}
                    />
                  </div>
                </div>

                {staffForm.baseSalary && (
                  <div className="pt-2 border-t border-blue-100/50 flex justify-between items-center text-[10px] font-bold text-blue-600 italic">
                    <span>Thực nhận dự kiến (Sau thuế 10%):</span>
                    <span className="text-sm font-black">${HumanResourceManager.calculateDynamicPay(staffForm.baseSalary, staffForm.kpiScore || 0, staffForm.overtimeHours || 0).net.toLocaleString()}</span>
                  </div>
                )}
              </div>
