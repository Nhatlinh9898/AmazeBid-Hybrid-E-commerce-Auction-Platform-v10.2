...
const RecruitmentModal: React.FC<RecruitmentModalProps> = ({ store, onClose }) => {
  ...
  const jobs = (store.jobPostings && store.jobPostings.length > 0) 
    ? store.jobPostings.filter(j => j.status === 'OPEN') 
    : defaultJobs;
      ...
      >
        ...
        <div className="p-6 bg-gray-50 border-t border-gray-100 text-center">
          <p className="text-xs text-gray-400 font-bold">
            Mọi thắc mắc vui lòng liên hệ Hotline: <span className="text-indigo-900">1900 xxxx</span>
          </p>
        </div>
      </motion.div>
    </div>
