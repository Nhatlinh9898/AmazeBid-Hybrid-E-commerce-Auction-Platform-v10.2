...
export const StoreManagement: React.FC<StoreManagementProps> = ({ 
  ...
  const [activeMainTab, setActiveMainTab] = React.useState<'MENU' | 'RECRUITMENT' | 'ANALYTICS'>('MENU');
  const [isAddingJob, setIsAddingJob] = React.useState(false);
  const [editingJob, setEditingJob] = React.useState<JobPosting | null>(null);
  const [jobForm, setJobForm] = React.useState<Partial<JobPosting>>({
    title: '',
    type: 'Full-time',
    salary: '',
    description: '',
  ...
  const handleAddJob = () => {
    ...
    const updatedJobs = [...(selectedStore.jobPostings || []), newJob];
    storeService.updateStore(selectedStore.id, { jobPostings: updatedJobs });
    setIsAddingJob(false);
    resetJobForm();
  };

  const handleUpdateJob = () => {
    if (!selectedStore || !editingJob) return;
    
    const updatedJobs = (selectedStore.jobPostings || []).map(job => 
  ...
  const handleDeleteJob = (jobId: string) => {
    if (!selectedStore || !window.confirm('Bạn có chắc muốn xóa tin tuyển dụng này?')) return;
    
    const updatedJobs = (selectedStore.jobPostings || []).filter(job => job.id !== jobId);
    storeService.updateStore(selectedStore.id, { jobPostings: updatedJobs });
  };
  ...
  const startEditStore = () => {
    ...
    setStoreForm({
      ...
      recruitmentInfo: selectedStore.recruitmentInfo || ''
    });
      ...
      {isAddingNewStore && stores.length === 0 && (
        <div className="bg-white rounded-2xl p-8 border border-blue-100 shadow-xl max-w-2xl mx-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Thông tin tuyển dụng chung</label>
              <textarea 
                placeholder="VD: Chúng tôi liên tục tuyển dụng vị trí phục vụ và pha chế..."
                className="w-full px-4 py-3 bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 h-24 resize-none"
                value={storeForm.recruitmentInfo}
                onChange={e => setStoreForm(prev => ({ ...prev, recruitmentInfo: e.target.value }))}
              />
            </div>
      ...
      {selectedStore && (
        <div className="flex gap-4 border-b border-gray-100 pb-0">
          <button 
            ...
            className={`px-6 py-3 font-black text-sm transition-all border-b-2 ${
              activeMainTab === 'MENU' 
      ...
      {selectedStore && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeMainTab === 'RECRUITMENT' && (
              <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm animate-in fade-in slide-in-from-left-4">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    <h3 className="text-xl font-black text-gray-900">Chiến dịch Tuyển dụng</h3>
                    <p className="text-sm text-gray-500">Quản lý các vị trí đang cần nhân sự tại {selectedStore.name}</p>
                  </div>
                  {hasPermission(StaffPermission.MANAGE_STAFF) && (
                    <button 
                      onClick={() => {
                      ...
                      className="bg-indigo-600 text-white px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg"
                    >
                ...
                <div className="space-y-4">
                  {(!selectedStore.jobPostings || selectedStore.jobPostings.length === 0) ? (
                    <div className="text-center py-12 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                      <Briefcase className="w-12 h-12 text-gray-300 mx-auto mb-3" />
                      <p className="text-gray-500 font-bold">Chưa có tin tuyển dụng nào.</p>
                      <button 
                        onClick={() => setIsAddingJob(true)}
                        className="text-indigo-600 text-sm font-black mt-2 hover:underline"
                      >
                        + Tạo tin đầu tiên ngay
                      </button>
                    </div>
                  ) : (
                    selectedStore.jobPostings.map(job => (
                      <div key={job.id} className="bg-white p-5 rounded-2xl border border-gray-100 flex items-center justify-between group hover:shadow-md transition-all">
                        <div className="flex items-center gap-4">
                          <div className={`p-3 rounded-xl ${job.status === 'OPEN' ? 'bg-green-50 text-green-600' : 'bg-gray-50 text-gray-400'}`}>
                            <Briefcase size={24} />
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                            ...
                            <div className="flex gap-4 mt-1">
                              <span className="text-xs text-gray-500 flex items-center gap-1"><FileText size={12}/> {job.type}</span>
                              <span className="text-xs text-indigo-600 font-bold flex items-center gap-1"><CircleDollarSign size={12}/> {job.salary}</span>
                            </div>
                          </div>
                        </div>
            ...
            {activeMainTab === 'ANALYTICS' && (
              <RevenueAnalytics store={selectedStore} />
            )}
          </div>
          ...
          <div className="space-y-6">
            {isAddingJob ? (
              <div className="bg-white rounded-2xl p-6 border border-indigo-100 shadow-xl shadow-indigo-50 animate-in slide-in-from-right-4 sticky top-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="font-black text-gray-900 flex items-center gap-2">
                    <Briefcase size={18} className="text-indigo-600"/>
                    {editingJob ? 'Sửa tin tuyển dụng' : 'Đăng tin mới'}
                  </h3>
                  <button onClick={() => setIsAddingJob(false)} className="text-gray-400 hover:text-gray-600"><X size={20}/></button>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Vị trí tuyển dụng</label>
                    <input 
                      type="text" 
                      placeholder="VD: Phục vụ, Thu ngân, Pha chế..."
                      className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                      value={jobForm.title}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Hình thức</label>
                      <select 
                        className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                        value={jobForm.type}
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Lương / Quyền lợi</label>
                      <input 
                        type="text" 
                        placeholder="VD: 25k/h hoặc 8-10tr"
                        className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Mô tả công việc</label>
                    <textarea 
                      className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                      rows={3}
                      placeholder="Mô tả công việc và thời gian làm việc..."
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Yêu cầu (ngăn cách bằng dấu phẩy)</label>
                    <textarea 
                      className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                      rows={2}
                      placeholder="VD: Nhanh nhẹn, có kinh nghiệm, trung thực..."
                      value={jobForm.requirements?.join(', ')}
                      onChange={e => setJobForm(prev => ({ ...prev, requirements: e.target.value.split(',').map(s => s.trim()).filter(s => s) }))}
                    />
                  </div>

                  <div className="flex items-center gap-2">
                    <select 
                      className="flex-1 px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 mt-4"
                  >
                    <Save size={18} /> {editingJob ? 'Cập nhật tin' : 'Đăng tuyển ngay'}
                  </button>
                </div>
              </div>
            ) : isEditingStore ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea 
                      className="w-full px-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                      rows={3}
                      placeholder="Thông tin chung về tuyển dụng..."
                      value={storeForm.recruitmentInfo}
                      onChange={e => setStoreForm(prev => ({ ...prev, recruitmentInfo: e.target.value }))}
                    />
                  </div>
