...
const StoreDetail: React.FC<StoreDetailProps> = ({ storeId, onClose }) => {
  ...
  const submitReview = async () => {
    ...
    try {
      const newReview: any = {
        ...
        userAvatar: user?.avatar || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=100&auto=format&fit=crop',
        rating: reviewRating,
      ...
      storeService.addReview(storeId, newReview);
      ...
      <div className="fixed inset-0 z-[300] bg-white overflow-y-auto">
        ...
        <div className="hidden print:block p-8 text-black bg-white min-h-screen font-sans">
          <div className="text-center mb-8">
            ...
            <p className="text-sm">SĐT: Liên hệ cửa hàng</p>
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-white overflow-y-auto">
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                    ...
                    <div className="flex gap-4">
                      <div className="text-right">
                        ...
                        <p className="text-lg font-black">Liên hệ cửa hàng</p>
                      </div>
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    ...
                    <div className="flex items-center gap-1 text-yellow-400 my-1">
                      {[1, 2, 3, 4, 5].map(star => (
                        <Star key={star} className={`w-4 h-4 ${star <= Math.round(store.rating) ? 'fill-current' : 'text-gray-200'}`} />
                      ))}
                    </div>
                  ...
                  <button 
                    ...
                    className="px-6 py-3 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-100"
                  >
                ...
                {showReviewForm && (
                  <div className="bg-blue-50/50 rounded-3xl p-6 border border-blue-100 mb-8 space-y-4">
                    ...
                    <textarea
                      ...
                      placeholder="Chia sẻ trải nghiệm của bạn về cửa hàng này..."
                      className="w-full h-32 bg-white rounded-2xl p-4 border border-gray-100 focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm transition-all shadow-inner"
                    />
                    <div className="flex justify-end">
                      <button
                        onClick={submitReview}
                        disabled={submittingReview || !reviewComment.trim()}
                        className="px-8 py-3 bg-blue-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
                      >
                        {submittingReview ? (
                          <>
                ...
                <div className="space-y-4">
                  {(store.reviews && store.reviews.length > 0) ? (
                    store.reviews.map(review => (
                      <div key={review.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm transition-all hover:shadow-md">
                    ...
                    <div className="py-20 text-center bg-gray-50 rounded-[40px] border-2 border-dashed border-gray-200">
                      <LucideStore className="w-16 h-16 text-gray-300 mx-auto mb-4 opacity-50" />
                      <p className="text-gray-400 font-bold">Chưa có đánh giá nào cho cửa hàng này</p>
                      <p className="text-xs text-gray-300 mt-2">Hãy là người đầu tiên để lại cảm nhận của bạn!</p>
                    </div>
                  )}
                </div>
              </div>
      ...
      {store && <StoreAIConsultant store={store} />}
    </div>
  );
};
