              ...
              <div className="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  {withdrawAmount && !isNaN(parseFloat(withdrawAmount)) && (
                    <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/50 space-y-2 animate-in fade-in slide-in-from-top-1">
                       ...
                       {(() => {
                         const breakdown = FinancialEngine.calculateTransactionBreakdown(parseFloat(withdrawAmount));
                         return (
                           <>
                             <div className="flex justify-between text-[11px] font-bold text-red-500 uppercase tracking-wider">
                                <span>Phí giao dịch (Đã trừ ưu đãi P2P)</span>
                                <span>-${breakdown.fees.toLocaleString()}</span>
                             </div>
                             <div className="pt-2 border-t border-indigo-100 flex justify-between text-sm font-black text-indigo-700 uppercase tracking-tight">
                                <span>Thực nhận về ngân hàng</span>
                                <span>${breakdown.net.toLocaleString()}</span>
                             </div>
                             <div className="text-[9px] text-indigo-400 italic font-medium">
                                * Tiền sẽ được chuyển vào tài khoản ngân hàng của bạn trong vòng {breakdown.escrowHoldDays} ngày làm việc.
                             </div>
                           </>
                         );
                       })()}
