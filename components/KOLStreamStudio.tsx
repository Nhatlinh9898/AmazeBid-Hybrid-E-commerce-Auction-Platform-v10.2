...
const KOLStreamStudio: React.FC<KOLStreamStudioProps> = ({ isOpen, onClose, onPostToFeed, products = [] }) => {
  ...
  const handleRestoreAsset = (asset: SavedAsset) => {
    if (asset.type === 'AVATAR') {
      const avatarIndex = kolGallery.indexOf(asset.url);
      if (avatarIndex !== -1) {
          ...
          <div className="p-8 max-w-2xl mx-auto">
            ...
            {activeTab === 'PREVIEW' && (
              <div className="space-y-6 animate-in slide-in-from-right-4 fade-in text-center">
                ...
                <div className="bg-[#1a1c1e] p-6 rounded-[2rem] border border-gray-800 shadow-xl max-w-2xl mx-auto mb-8 relative overflow-hidden text-left">
                  ...
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h4 className="text-sm font-black text-white uppercase tracking-widest">Dự báo hiệu năng AI</h4>
                      <p className="text-[10px] text-gray-500 font-bold">Thuật toán KOLAnalytics v2.4 (Real-time Prediction)</p>
                    </div>
                  </div>

                  {(() => {
                    const stats = KOLAnalytics.predictStreamPerformance(1250, 450, user?.reputation || 5000);
                    return (
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[9px] text-gray-500 font-black uppercase mb-1">Tương tác</p>
                          <p className="text-xl font-black text-white">{stats.engagementScore}</p>
                        </div>
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[9px] text-gray-500 font-black uppercase mb-1">Chuyển đổi</p>
                          <p className="text-xl font-black text-indigo-400">{stats.conversionRate}%</p>
                        </div>
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[9px] text-gray-500 font-black uppercase mb-1">Đơn dự kiến</p>
                          <p className="text-xl font-black text-emerald-400">~{stats.estimatedOrders}</p>
                        </div>
                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                          <p className="text-[9px] text-gray-500 font-black uppercase mb-1">Doanh thu ước tính</p>
                          <p className="text-xl font-black text-amber-400">${stats.potentialRevenue}</p>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  <div className="p-4 bg-indigo-500/5 rounded-2xl border border-indigo-500/10 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-[10px] text-indigo-300 font-bold leading-relaxed">
                        "Kịch bản AI đã được tối ưu hóa cho phong cách <strong>{kolStyle}</strong>. Dựa trên dữ liệu, video này có khả năng viral cao gấp 2.4 lần so với kịch bản tiêu chuẩn."
                      </p>
                    </div>
                  </div>
                </div>
