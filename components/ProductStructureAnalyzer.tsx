...
const isDeepMatch = (item: BOMItem, term: string): boolean => {
  ...
  const selfMatch = item.name.toLowerCase().includes(lowerTerm) || item.productId.toLowerCase().includes(lowerTerm);
  if (selfMatch) return true;
  return item.subComponents?.some(sub => isDeepMatch(sub, term)) || false;
};
...
const ProductStructureAnalyzer: React.FC = () => {
  ...
  const filteredBoms = boms.filter(b => isDeepMatch(b, searchTerm));

  const performSmartSearch = async () => {
    if (!searchTerm) return;
    setIsSmartSearch(true);
    try {
      const bomData = boms.map(b => ({ name: b.name, id: b.id, specs: b.specs }));
      const prompt = `Dựa trên danh sách sản phẩm sau: ${JSON.stringify(bomData)}. 
      Tìm các sản phẩm liên quan đến yêu cầu: "${searchTerm}". 
      Chỉ trả về danh sách ID (phân cách bằng dấu phẩy) của các sản phẩm phù hợp nhất. Nếu không có gì khớp, trả về "NONE".`;
      
      const response = await geminiService.generateText(prompt);
      if (response.trim() !== 'NONE') {
        const matchingIds = response.split(',').map(s => s.trim());
        const result = boms.filter(b => matchingIds.includes(b.id));
        if (result.length > 0) {
          setSelectedBOM(result[0]);
        }
      }
    } catch (err) {
  ...
  React.useEffect(() => {
    if (searchTerm) {
      ...
      const findNodesToExpand = (items: BOMItem[]) => {
        items.forEach(item => {
          const childMatch = item.subComponents?.some(sub => isDeepMatch(sub, searchTerm));
          if (childMatch) {
            newExpanded.add(item.id);
            if (item.subComponents) findNodesToExpand(item.subComponents);
          }
        });
      };
      findNodesToExpand(boms);
    ...
    return (
      <div key={item.id} className="ml-4 border-l border-slate-800">
        <div 
          ...
          className={`group flex items-center gap-3 p-3 rounded-xl cursor-all transition-all hover:bg-slate-800/50 ${selectedBOM?.id === item.id ? 'bg-indigo-500/10 border-l border-indigo-500' : ''} ${isMatched ? 'bg-indigo-500/5 ring-1 ring-indigo-500/20' : ''}`}
        >
          {hasChildren ? (
          ...
          <div className="flex-1 flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <span className={`text-sm font-bold ${isMatched ? 'text-indigo-400' : 'text-slate-200'}`}>{item.name}</span>
        ...
        <div className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6 backdrop-blur-xl">
          ...
          <div className="relative mb-6 flex gap-2">
            ...
            <button
              ...
              disabled={isSmartSearch || !searchTerm}
              className={`p-3 rounded-2xl border flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${isSmartSearch ? 'bg-indigo-500/20 border-indigo-500/30 text-indigo-400' : 'bg-slate-950/50 border-slate-800 text-slate-400 hover:border-indigo-500/50 hover:text-indigo-400'}`}
            >
              <Wand2 size={14} className={isSmartSearch ? 'animate-spin' : ''} />
              {isSmartSearch ? 'Đang phân tích...' : 'Thông minh'}
            </button>
          </div>
          ...
          <>
            ...
            <div className="bg-slate-900/50 rounded-3xl border border-slate-800 p-6 backdrop-blur-xl">
              <h3 className="text-md font-black text-white flex items-center gap-2 uppercase tracking-wider mb-6">
                <Scale className="text-indigo-400" size={18} /> Phân Tích Định Mức (Recipe)
              </h3>
              
              <div className="space-y-4">
                <div className="bg-slate-950/40 p-5 rounded-2xl border border-slate-800">
                  <div className="flex justify-between items-center mb-4">
                    <span className="text-xs font-bold text-slate-300">Tính toán giá thành sản xuất (Dữ liệu thuật toán)</span>
                    <span className="text-[10px] text-indigo-400 bg-indigo-500/10 px-2 py-1 rounded-full border border-indigo-500/20">SupplyChain v1.0</span>
                  </div>
                  
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    <div className="space-y-1">
                      <p className="text-[10px] text-slate-500 uppercase font-black">Giá vật tư (Gốc)</p>
                      <p className="text-lg font-black text-white">${selectedBOM.basePrice}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-slate-500 uppercase font-black">Hao hụt dự kiến (5%)</p>
                      <p className="text-lg font-black text-red-400">+${(selectedBOM.basePrice * 0.05).toFixed(2)}</p>
                    </div>
                    <div className="space-y-1">
                      <p className="text-[10px] text-slate-500 uppercase font-black">Chi phí nhân công</p>
                      <p className="text-lg font-black text-blue-400">$15.00</p>
                    </div>
                    <div className="bg-indigo-500/10 p-3 rounded-xl border border-indigo-500/20">
                      <p className="text-[10px] text-indigo-400 uppercase font-black">Tổng giá thành</p>
                      <p className="text-xl font-black text-indigo-300">${(selectedBOM.basePrice * 1.05 + 15).toFixed(2)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-4 bg-amber-500/5 rounded-2xl border border-amber-500/20">
                  ...
                  <p className="text-[10px] text-amber-200/70 leading-relaxed italic">
                    "Thuật toán SupplyChain đã tính toán chi phí dựa trên hao hụt định mức và giá thầu tối ưu từ các nhà cung cấp nội bộ trong hệ sinh thái AmazeBid."
                  </p>
                </div>
              </div>
            </div>
