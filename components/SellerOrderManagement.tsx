...
const SellerOrderManagement: React.FC<SellerOrderManagementProps> = ({ products, currentUserId, globalConfig }) => {
  ...
  const allOrders = Array.from(allOrdersMap.values()).sort((a, b) => {
    const priorityA = OrderLogic.getOrderPriority(a) === 'URGENT' ? 1 : 0;
    const priorityB = OrderLogic.getOrderPriority(b) === 'URGENT' ? 1 : 0;
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-1">
                        ...
                        {OrderLogic.getOrderPriority(order) === 'URGENT' && (
                          <span className="flex items-center gap-1 text-[8px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded uppercase">
                            <AlertTriangle size={8} /> ƯU TIÊN
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center bg-gray-50 p-3 rounded-xl">
                         ...
                         <span className="text-sm font-black text-right shrink-0">
                           {selectedOrder.items.reduce((sum, i) => sum + (i.price * (i.quantity || 1)), 0).toLocaleString()}<sup>{globalConfig.currencySymbol}</sup>
                         </span>
                      </div>
                      <div className="flex justify-between items-center px-3">
                         ...
                         <span className="text-sm font-bold text-right shrink-0">
                           +{(selectedOrder.items.reduce((sum, i) => sum + (i.price * (i.quantity || 1)), 0) * globalConfig.defaultVatRate).toLocaleString()}<sup>{globalConfig.currencySymbol}</sup>
                         </span>
                      </div>
                      {!selectedOrder.isPOS && (
                        <div className="flex justify-between items-center px-3">
                         ...
                         <div className="flex justify-between items-end">
                            <div>
                               ...
                               <span className="text-2xl font-black text-blue-600">
                                 {selectedOrder.totalAmount.toLocaleString()}<sup>{globalConfig.currencySymbol}</sup>
                               </span>
                            </div>
                            <div className="text-right">
                               <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">Dự tính doanh thu</p>
                               <span className="text-sm font-black text-emerald-600">
                                 +{(selectedOrder.items.reduce((sum, i) => sum + (i.price * (i.quantity || 1)), 0) * (1 - globalConfig.platformFeeRate)).toLocaleString()}<sup>{globalConfig.currencySymbol}</sup>
                               </span>
                            </div>
                         </div>
                      </div>
                      ...
                      <div className="mt-6 flex gap-2">
                        <div className={`flex-1 p-3 border rounded-xl flex items-center gap-3 ${selectedOrder.isPOS ? 'bg-amber-50 border-amber-100' : 'bg-indigo-50 border-indigo-100'}`}>
                           {selectedOrder.isPOS ? <ShoppingBag size={20} className="text-amber-600 shrink-0" /> : <ShieldCheck size={20} className="text-indigo-600 shrink-0" />}
                           <div>
                              <p className={`text-[10px] font-black uppercase ${selectedOrder.isPOS ? 'text-amber-700' : 'text-indigo-700'}`}>{selectedOrder.isPOS ? 'Direct Sale (POS)' : 'Escrow Locked'}</p>
                              <p className={`text-[9px] leading-tight ${selectedOrder.isPOS ? 'text-amber-500' : 'text-indigo-500'}`}>
                                {selectedOrder.isPOS 
